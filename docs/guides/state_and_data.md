...
- **2026-05-29 / 2026-06-11 (Invite Share RLS Cleanup):** [20260529150000_invite_rls_fixes.sql](../../supabase/migrations/20260529150000_invite_rls_fixes.sql) now removes the obsolete browser-callable invite counter RPC and tightens `group_members_insert` so project leaders can add regular members but cannot mint new leaders through direct inserts. Invite redemption and approval counters are handled by the service-only `consume_project_invite` / `approve_project_join_request` flow.
...
- **2026-06-11 (Project Sharing Verification & E2E Testing):** Verified the 3 SQL migrations to ensure they correctly implement security definer functions, restricted invite visibility, and student/lecturer role onboarding protections. Executed end-to-end browser tests via the `browser` subagent to register a group leader (`user_a_leader@test.com`), create a project ("Google Style Project"), change visibility to "Anyone with the link", register a student member (`user_b_member@test.com`), request to join using the invite code (`IV-690YAJ`), approve the request as the leader, and confirm the member was successfully added.
- **2026-06-11 (Rubric Grade Submission Notification):** Fixed missing notification dispatch when a lecturer submits a rubric grade. After `saveRubricGrade("submitted")` succeeds, the code now fetches group members via `fetchGroupMembers` and sends each one a bilingual notification with the lecturer name, rubric name, and score. Failures are caught and logged without blocking the grade submission. Changed files: [LecturerRubricGrade.tsx](../../src/pages/LecturerRubricGrade.tsx).
- **2026-06-11 (Student Evaluation Panel Group Selector):** Fixed the Student Evaluation sidebar sending notifications to wrong recipients. Root cause: the panel used `currentGroupIndex` (designed for student workspace) instead of an explicit group selector, causing member lookups to target the wrong group. Added a group/project dropdown, derived member options from the selected group, and guarded `sendNotification` to only fire when a valid UUID is resolved. Changed files: [LecturerStudentEvaluationPanel.tsx](../../src/components/feature-groups/LecturerStudentEvaluationPanel.tsx).
- **2026-06-11 (Nullable lecturer_id — Student Project Creation Fix):** Fixed bug where student-created projects auto-assigned the student as the group's lecturer. `createPersistedGroup` now accepts an optional `creatorRole` parameter; `lecturer_id` is set to `null` for students and only populated for lecturers/admins. Migration [20260611140000_nullable_lecturer_id.sql](../../supabase/migrations/20260611140000_nullable_lecturer_id.sql) drops the `NOT NULL` constraint, updates `groups_insert` and `group_members_insert` RLS policies to handle `NULL`, and backfills existing student-created groups. Changed files: [teamPersistence.ts](../../src/lib/teamPersistence.ts), [TeamContext.tsx](../../src/context/TeamContext.tsx).

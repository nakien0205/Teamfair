...
- Contribution and fairness
  ...
  - [src/pages/StudentMyContribution.tsx](../../src/pages/StudentMyContribution.tsx) - Full contribution score page with AI-powered analysis. Shows reference score, AI effort summary, anomaly detection, recommendations, score breakdown with weight redistribution when data is missing, evidence summary, and leader feedback. AI analysis only runs when the student clicks the AI analysis button, then caches results in Supabase `contribution_ai_analysis` table.
  - [src/lib/studentContribution.ts](../../src/lib/studentContribution.ts) - Deterministic score calculation. When peer review or leader evaluation data is missing, their weight (10% each) is redistributed proportionally to other components instead of fabricating 60% defaults. Each breakdown item has a `hasData` boolean.
  - [src/lib/contributionAi.ts](../../src/lib/contributionAi.ts) - AI analysis layer: fetches from Python agent server (`POST /analyze-contribution`), caches results in Supabase with hash-based staleness detection.
